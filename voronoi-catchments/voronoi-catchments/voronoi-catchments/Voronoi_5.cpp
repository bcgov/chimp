...
Voronoi_5::Voronoi_5(string inFile, string outFile)
{
	_inFile = inFile;
	_outFile = outFile;
	_precision = 11;
...
void Voronoi_5::process()
{
	...
	std::vector<SDG2_Voronoi_5::Site_2> sites;
	while (ifs >> site) {
		recordSite(site);
	...
	SDG2_Voronoi_5::Finite_edges_iterator eit = sdg.finite_edges_begin();
...
*/
array<double, 4> Voronoi_5::estimateOutputBbox() {
	array<double, 4> bbox;
...
void Voronoi_5::recordPoint(SDG2_Voronoi_5::Point_2 point) {
...
void Voronoi_5::recordSite(SDG2_Voronoi_5::Site_2 site) {
	if (site.is_segment()) {
...
*/
double Voronoi_5::integerDivisionToDouble(string integerDivisionAsString) {
...
int Voronoi_5::getSplitIndex(string token, array<double, 4> outputBbox) {
...
string Voronoi_5::cleanLineWithFractions(string line, array<double, 4> bbox) {
	string cleanedLine = "";
	string delim = " ";
	size_t prev = 0, pos = 0;
...
*/
string Voronoi_5::lineToWkt(string line) {
